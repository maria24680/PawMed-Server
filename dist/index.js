...
const connectDB = async () => {
    try {
        client = new mongodb_1.MongoClient(process.env.MONGODB_URI || 'mongodb://localhost:27017/pawmed');
...
function lookupUser(localField, as, preserveNull = true) {
    return [
        {
            $lookup: {
                from: 'user',
...
function lookupPet(localField, as, preserveNull = true) {
    return [
        {
            $lookup: {
                from: 'pets',
...
app.put('/api/auth/profile', auth, async (req, res) => {
    try {
        ...
        const result = await Users.findOneAndUpdate({ _id: userId }, { $set: { name, phone, address, profileImage } }, { returnDocument: 'after' });
        if (!result) {
...
app.put('/api/pets/:id', auth, async (req, res) => {
    try {
        ...
        if (!petId)
            return res.status(400).json({ message: 'Invalid pet id' });
        const result = await Pets.findOneAndUpdate({ _id: petId }, { $set: { ...req.body, updatedAt: new Date() } }, { returnDocument: 'after' });
        if (!result)
            return res.status(404).json({ message: 'Pet not found' });
        return res.status(200).json({
            success: true,
...
app.put('/api/appointments/:id/status', auth, roleCheck('veterinarian', 'admin'), async (req, res) => {
    try {
        const apptId = toObjectId(req.params.id);
        if (!apptId)
            return res.status(400).json({ message: 'Invalid appointment id' });
        const { status, notes } = req.body;
        const result = await Appointments.findOneAndUpdate({ _id: apptId }, { $set: { status, notes, updatedAt: new Date() } }, { returnDocument: 'after' });
        if (!result)
            return res.status(404).json({ message: 'Appointment not found' });
        const populated = await Appointments.aggregate([
            { $match: { _id: apptId } },
...
app.put('/api/appointments/:id/cancel', auth, async (req, res) => {
    try {
        ...
        if (!apptId)
            return res.status(400).json({ message: 'Invalid appointment id' });
        const result = await Appointments.findOneAndUpdate({ _id: apptId }, { $set: { status: 'cancelled', updatedAt: new Date() } }, { returnDocument: 'after' });
        if (!result)
            return res.status(404).json({ message: 'Appointment not found' });
        return res.status(200).json({
            success: true,
...
app.post('/api/services', auth, roleCheck('admin'), async (req, res) => {
    try {
        const doc = {
            ...req.body,
...
app.put('/api/services/:id', auth, roleCheck('admin'), async (req, res) => {
    try {
        const serviceId = toObjectId(req.params.id);
        if (!serviceId)
            return res.status(400).json({ message: 'Invalid service id' });
        const result = await Services.findOneAndUpdate({ _id: serviceId }, { $set: { ...req.body, updatedAt: new Date() } }, { returnDocument: 'after' });
        if (!result)
            return res.status(404).json({ message: 'Service not found' });
        return res.status(200).json({
            success: true,
...
app.delete('/api/services/:id', auth, roleCheck('admin'), async (req, res) => {
    try {
        const serviceId = toObjectId(req.params.id);
        if (!serviceId)
            return res.status(400).json({ message: 'Invalid service id' });
...
// PRESCRIPTION ROUTES
// ============================================
app.post('/api/prescriptions', auth, roleCheck('veterinarian', 'admin'), async (req, res) => {
    try {
...
app.put('/api/prescriptions/:id', auth, roleCheck('veterinarian', 'admin'), async (req, res) => {
    try {
        const presId = toObjectId(req.params.id);
        if (!presId)
            return res.status(400).json({ message: 'Invalid prescription id' });
        const result = await Prescriptions.findOneAndUpdate({ _id: presId }, { $set: { ...req.body, updatedAt: new Date() } }, { returnDocument: 'after' });
        if (!result)
            return res.status(404).json({ message: 'Prescription not found' });
        return res.status(200).json({
            success: true,
...
app.delete('/api/prescriptions/:id', auth, roleCheck('veterinarian', 'admin'), async (req, res) => {
    try {
        const presId = toObjectId(req.params.id);
        if (!presId)
            return res.status(400).json({ message: 'Invalid prescription id' });
...
app.post('/api/payments', auth, async (req, res) => {
    try {
        ...
        await Appointments.updateOne({ _id: apptObjectId }, { $set: { paymentStatus: 'paid', paymentId: generatedStripeId, amount: doc.amount, updatedAt: new Date() } });
...
app.get('/api/payments/appointment/:appointmentId', auth, async (req, res) => {
    try {
...
// ============================================
app.post('/api/medicalrecords', auth, roleCheck('veterinarian', 'admin'), async (req, res) => {
    try {
...
app.get('/api/medicalrecords', auth, async (req, res) => {
    try {
...
app.get('/api/medicalrecords/:id', auth, async (req, res) => {
    try {
...
app.put('/api/medicalrecords/:id', auth, roleCheck('veterinarian', 'admin'), async (req, res) => {
    try {
        const recordId = toObjectId(req.params.id);
        if (!recordId)
            return res.status(400).json({ message: 'Invalid medical record id' });
        const result = await MedicalRecords.findOneAndUpdate({ _id: recordId }, { $set: { ...req.body, updatedAt: new Date() } }, { returnDocument: 'after' });
        if (!result)
            return res.status(404).json({ message: 'Medical record not found' });
        return res.status(200).json({
            success: true,
...
app.delete('/api/medicalrecords/:id', auth, roleCheck('veterinarian', 'admin'), async (req, res) => {
    try {
        const recordId = toObjectId(req.params.id);
        if (!recordId)
            return res.status(400).json({ message: 'Invalid medical record id' });
...
// ADMIN ROUTES
// ============================================
app.get('/api/admin/dashboard', auth, roleCheck('admin'), async (_req, res) => {
    try {
        const [totalUsers, totalPets, totalAppointments, totalServices, totalPrescriptions, totalPayments, pendingAppointments] = await Promise.all([
...
app.get('/api/admin/users', auth, roleCheck('admin'), async (_req, res) => {
    try {
...
app.put('/api/admin/users/:id/role', auth, roleCheck('admin'), async (req, res) => {
    try {
        const { role } = req.body;
        ...
        const filter = objId ? { $or: [{ _id: idParam }, { _id: objId }] } : { _id: idParam };
        const result = await Users.findOneAndUpdate(filter, { $set: { role } }, { returnDocument: 'after' });
        if (!result)
            return res.status(404).json({ message: 'User not found' });
        return res.status(200).json({
            success: true,
...
app.put('/api/admin/users/:id/toggle', auth, roleCheck('admin'), async (req, res) => {
    try {
        const idParam = req.params.id;
        const objId = toObjectId(idParam);
        const filter = objId ? { $or: [{ _id: idParam }, { _id: objId }] } : { _id: idParam };
        ...
        const result = await Users.findOneAndUpdate(filter, { $set: { isActive: newActive } }, { returnDocument: 'after' });
        return res.status(200).json({
            success: true,
...
// VETERINARIAN ROUTES
// ============================================
app.get('/api/veterinarian/appointments', auth, roleCheck('veterinarian'), async (req, res) => {
    try {
        const { status, date } = req.query;
        const match = { veterinarian: req.user?._id };
        if (status)
            match.status = status;
        if (date)
...
app.get('/api/veterinarian/stats', auth, roleCheck('veterinarian'), async (req, res) => {
    try {
        const vetId = req.user?._id;
        const todayStart = new Date(new Date().setHours(0, 0, 0, 0));
        const todayEnd = new Date(new Date().setHours(23, 59, 59, 999));
        const [totalAppointments, completedAppointments, pendingAppointments, todayAppointments, totalPrescriptions] = await Promise.all([
